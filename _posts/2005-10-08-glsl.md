...
以下のプログラムでは、`gl_TexCoord[0]` に格納されているテクスチャ座標をテクスチャ変換行列 `gl_TextureMatrix[0]` により変換し、その結果を使って `color` で指定されたテクスチャユニットが保持するテクスチャをサンプリングします。そして、サンプリングした色 `fcolor` を使って拡散反射光強度と環境光の反射光強度を計算します。
