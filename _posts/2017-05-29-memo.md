...
category: VR
tags: [GLFW, OpenGL, VR, Oculus Rift]
published: true
---
...
とはいっても、[GLFW](https://www.glfw.org/) くらいは使います。これくらいは使わせてください。

```cpp
#include <GLFW/glfw3.h>
```

このほか、当然ながら [Oculus SDK for Windows](https://developer.oculus.com/downloads/native-windows/) に含まれる LibOVR を使用します[^3]。SDK のバージョンは Oculus Rift DK1 / DK2 で使われる 0.8 と CV1 で使われる 1.x (これを書いている時点では 1.14) に対応しています。この切り替えは OVR_PRODUCT_VERSION で判断しています。また、GLFW で LibOVR を使う場合、ヘッダファイルの読み込みに若干工夫が必要です。このほか、これより前に `USE_OCULUS_RIFT` が `#define` されているものとします。 
...
その後、`ovr_Create()` を呼び出して Oculus Rift のセッションを作成します。その際、`ovr_Create()` から得られた `luid` を冒頭で作成した `GetDefaultAdapterLuid()` の値と比較して、一致していなければ (`Compare()` で呼び出している `memcmp()` の値が非 0) ならエラーとします。
 
```cpp
      // LUID は Oculus SDK 0.8 では使っていないらしい
...
FBO のレンダーターゲットを作成します。Oculus SDK 1.x では、まず `ovr_CreateTextureSwapChainGL()` でテクスチャのリストみたいなものを作るようです。その際に `ovrTextureSwapChainDesc` 構造体によってテクスチャの特性を指定します。ここでミップマップレベルやマルチサンプリングなんかも指定できるようです (やってません)。そして、`ovr_GetTextureSwapChainLength()` でそのリストの長さを求め、その数だけ `ovr_GetTextureSwapChainBufferGL()` でテクスチャを作成します。`ovr_GetTextureSwapChainBufferGL()` はテクスチャ名を `texId` に返すので、それを `glBindTexture()` して `glTexParameteri()` でテクスチャのパラメータを設定します。
...
デプスバッファについては、Oculus SDK 1.x では普通にデプステクスチャを目ごとに一枚作成します。
...
Oculus Rift に描画する際は、それに先立ってフレームごとに Oculus Rift の状態を取得し、それに対応した処理をします。Oculus SDK 1.x では、HMD を介してアプリケーションの終了要求を受け取った時に、アプリケーションを終了します。ここでは GLFW を使っているので、`glfwSetWindowShouldClose()` を使って描画ループを終了させています。また、Oculus Rift CV1 ではユーザが HMD を外したことを検知するので、その場合は Oculus Rift にレンダリングを行わないようにします。ここでは `false` を返してそのことを呼び出し側に伝えています。あと、Oculus SDK 1.x では Asynchronous TimeWarp 処理のために、フレームごとに視点の位置を調整するので、ここで視点の位置を取得しておきます。
...
なお、Oculus SDK 0.8 では TimeWarp 処理のために、ここでスクリーンの大きさ `screen` と前方面・後方面の位置 (`zNear`, `zFar`) で求めた透視投影変換行列 (`glFrustum()` で求められるもの) の要素を取り出しておく必要があります。この透視投影変換行列を `projection[4][4]` とするとき、これを参照して以下の処理を行います。
...
これらが完了した後、ミラー表示を行っていた OpenGL (GLFW) のウィンドウを削除します。
...
[^2]: 実は前に Qiita に書いた ([その1](https://qiita.com/tokoik/items/61ae07d0f8f448959deb)) ([その2](https://qiita.com/tokoik/items/1b4bc645786632513773)) のですが、改めて自分のブログにも書きました。
