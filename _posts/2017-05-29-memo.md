...
最初に o`vr_Initialize()` を呼び出して、LibOVR を初期化します。これに失敗したときは、ここでは単に return していますが、例外を投げるなどエラー処理を適切に行ってください。成功したら、プログラムの終了時に呼び出す必要のある `ovr_Shutdown()` を `atexit()` で登録しておきます。
