...
また、視点座標系の基底ベクトル (軸ベクトル) **x'**, **y'**, **z'** を求めます。**z'** は視線の逆ベクトルを正規化して $\mathbf{z}' = (\mathbf{e} - \mathbf{t}) / \vert\mathbf{e} - \mathbf{t}\vert$, **x'** は $\mathbf{u}$ と **z'** に対して垂直なので $\mathbf{x}' = (\mathbf{u} \times \mathbf{z}') / \vert\mathbf{u} \times \mathbf{z}'\vert$, **y'** は **z'** と **x'** に垂直なので $\mathbf{y}' = \mathbf{z}' \times \mathbf{x}'$ となります。
