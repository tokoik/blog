---
title: "OS X 10.9 / Xcode 5 で GLUT を使うと警告が出る"
date: 2014-06-17 00:00:00 +0900
category: メモ
tags: [OpenGL, GLUT, GLFW, macOS, Xcode]
published: true
---
...
それで、ここは [GLFW](http://www.glfw.org/) に移行すべきかと思って[こんなの]({{ site.baseurl }}{% post_url 2012-09-06-glfw %})を書いたり、GLFW も 3 に移っちゃってこの記事がいきなり古くなっちゃって、慌てて[こんなの]({{ site.baseurl }}/assets/pdfs/GLFW.pdf)を書いたりしていました。
...
- コマンドラインでは `-mmacosx-version-min=10.8` というオプションを追加するか、環境変数 `MACOSX_DEPLOYMENT_TARGET` に `10.8` を設定しておく[^1]。

[^1]: OpenGL 関係のヘッダファイルを `#include` する前に、`GL_SILENCE_DEPRECATION` を `#define` すればいいです。(2026 年 8 月 30 日 追記)
