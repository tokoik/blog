...
「[『OpenGL の勉強をする』という言い方には違和感がある](http://twitter.com/tokoik/status/29363340050)」とか言ってる割に、今 Transform Feedback の使い方を「[勉強している]({{ site.baseurl }}{% post_url 2011-08-18-memo %})」ところなのですが、その時に作った[サンプルプログラム](https://github.com/tokoik/tfb)を、「オレオレ補助ライブラリ (Gg.h/Gg.cpp)」を使って書き直しました。あと `glBufferData()` の usage を `GL_STATIC_DRAW` にしてましたけど、twitter で「`GL_DYNAMIC_COPY` じゃないのか」という指摘があったので、そこも変更しました (動作は変わんないんですけど)。このプログラムをベースに解説します。なお、この「オレオレ補助ライブラリ」 は来年の[講義](https://tokoik.github.io/gg/)の宿題のベースにしようと今練っているところです。
