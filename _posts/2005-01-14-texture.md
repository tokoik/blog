---
title: "テクスチャ:第１１回 スフィアマッピングで Phong シェーディング"
date: 2005-01-14
...
![中間ベクトル h の求め方]({{ site.baseurl }}/assets/images/halfway.gif)
...
テクスチャの拡大には線形補間 `GL_LINEAR` を行うようにします．これでテクスチャのサイズが小さくても，鏡面反射を滑らかに見せることができます．またこのテクスチャは充分滑らかなので，テクスチャの縮小は最近傍法 `GL_NEAREST` のままでも問題ないと思いますが，これにも線形補間 `GL_LINEAR` を用いることにします．

テクスチャによる鏡面反射光は，OpenGL の陰影付けによる拡散反射光に加算して合成します．このために，[`glTexEnvi()`](https://registry.khronos.org/OpenGL-Refpages/gl2.1/xhtml/glTexEnv.xml) で `GL_TEXTURE_ENV_MODE` に `GL_ADD` を指定します．これは OpenGL 1.3 から標準機能に取り込まれた機能なので，ビデオコントローラによっては機能しない場合があるようです（ATI Mobility M4 を搭載している ThinkPad や SGI VWS320 では機能しませんでした）．
...
ということで，見事に Phong のスムーズシェーディングらしくなっています．また図形を回転させると，Gouraud のスムーズシェーディングの問題の一つである[鏡面反射の消失](https://tokoik.github.io/cg/cgsample11fig.html)が発生していないことがわかります．
