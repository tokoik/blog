...
ただし、このままだと点の密度が中心からの距離 $r$ に反比例して下がってしまうので、$r$ として一様乱数 $u$ の代わりに $\sqrt{2u}$ を用います。これは $r$ に比例した分布を持つ乱数になります。
...
これにより、$u$ が区間 $[0, 1]$ に一様に分布する乱数であれば、半径 1, 高さ 1 の円柱状の空間に点が散布されます。
...
> $$
>
> の逆関数 $F^{-1}(x)$ の引数に一様乱数 $u$ を与えます。この方法は逆関数法と呼ばれます。$f(x) = x$ なら、
>
> $$
> F^{-1}(x) = \sqrt{2x}
> $$
>
> また球状に散布する場合は、$f(x) = x^2$ なので、
>
> $$
> F^{-1}(x) = \sqrt[3]{3x}
> $$
