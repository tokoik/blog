...
さてさて今回は、[前回]({{ page.previous | relative_url }})の途中でくじけたキューブマッピングによる Phong シェーディングの実装を行います。雛型には[キューブマッピングでテクスチャを回転]({{ site.baseurl }}{% post_url 2005-01-31-texture %})したときに作成したものを用います。
