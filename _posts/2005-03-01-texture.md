...
date: 2005-03-01
category: テクスチャ入門
tags: [OpenGL, ゼミ, テクスチャ, キューブマッピング]
published: true
---
...
![キューブマッピングにおける法線ベクトル]({{ site.baseurl }}/assets/images/texture/cubemap8.jpg)
...
あと、今回テクスチャに割り当てる画像にはアルファチャンネルを付けていないので、テクスチャを割り当てる際には画像の書式に `GL_RGB` を指定してください。また、６枚の画像が配列変数 `textures` のひとつひとつの要素に格納されているので、`textures` に添え字 `[i]` を付けておいてください。
...
![キューブマッピングで Phong シェーディング]({{ site.baseurl }}/assets/images/texture/cubemap9.jpg)
...
![鏡面反射光成分と拡散反射光成分が逆に動く]({{ site.baseurl }}/assets/images/texture/cubemap10.jpg)
...
なお Windows でこの機能を使う場合には、少しおまじないが必要になります。先にその部分を説明します。まず、プログラムの最初の部分で [glext.h](https://registry.khronos.org/OpenGL/api/GL/glext.h) を `#include` している部分の後で、[`glMultTransposeMatrixd()`](https://registry.khronos.org/OpenGL-Refpages/gl2.1/xhtml/glMultTransposeMatrix.xml) という関数の関数ポインタ変数 `glMultTransposeMatrixd` を宣言しておきます。
...
![鏡面反射光成分と拡散反射光成分を同じ方向に回した場合]({{ site.baseurl }}/assets/images/texture/cubemap11.jpg)

- [鏡面反射光成分と拡散反射光成分を同じ方向に回したサンプルプログラム](https://github.com/tokoik/texture9)

| テクスチャを回した場合 | ティーポットを回した場合 |
| :---: | :---: |
| ![テクスチャを回した場合]({{ site.baseurl }}/assets/images/texture/texture9a.webp) | ![ティーポットを回した場合]({{ site.baseurl }}/assets/images/texture/texture9b.webp) |
