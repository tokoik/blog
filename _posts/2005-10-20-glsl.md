...
[前回]({{ page.previous | relative_url }})のシェーダ anisotropic.vert と anisotropic.frag を、それぞれ shlick.vert と shlick.frag という名前でコピーします（アプリケーションプログラムが `readShaderSource()` で読み込むファイル名も、それに合わせて変更してください）。そして、フラグメントシェーダに関数 `shlick()` を定義して、`pow()` を置き換えます。
