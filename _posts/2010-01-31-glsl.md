...
風邪ひいて寝転がっていてふと思いついたのでメモ。原田研のひろぴこさんが曲面を評価するときに縞模様 (ゼブラパターン) の映り込みを表示してましたけど、これは [GLSL による環境マッピング]({{ site.baseurl }}{% post_url 2008-12-20-glsl %})のフラグメントシェーダのプログラムを以下のように書き換えるだけで実現できます。(2月1日 車の画像を追加)
