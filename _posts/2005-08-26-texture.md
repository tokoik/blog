...
`GL_DOT3_RGB` あるいは `GL_DOT3_RGBA` による内積計算は，テクスチャメモリに格納されている単位ベクトルに対して，この変換が行われていることを考慮します．したがって，これらによる画素の内積計算は，２つのテクスチャ $a_{0(r,g,b) }$ と $a_{1(r,g,b) }$ に対して，次式により行われます[^2]．

[^2]: [`glTexEnv*()`](https://registry.khronos.org/OpenGL-Refpages/gl2.1/xhtml/glTexEnv.xml) の "`GL_DOT3_RGB` or `GL_DOT3_RGBA`" の項目を参照してください．
...
![バンプマッピング]({{ site.baseurl }}/assets/images/texture20.webp)
