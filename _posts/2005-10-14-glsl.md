...
ファイル main.cpp に、シェーダプログラムを読み込む手続きを追加します。これは[第１回]({{ site.baseurl }}{% post_url 2005-10-06-opengl %})と同様です。まず glsl.h を `#include` して、プログラムオブジェクト名に使う変数 `gl2Program` を宣言します。
...
バンプマッピングでは接空間の基底ベクトル、すなわち法線ベクトル $\mathbf{n}$、接線ベクトル $\mathbf{t}$、および従接線ベクトル $\mathbf{b}$ を使います。法線ベクトル $\mathbf{n}$ は図形の頂点属性として与えられており、従接線ベクトルは $\mathbf{b} = \mathbf{n}\times\mathbf{t}$ で求めることができるので、このプログラムオブジェクトには、接線ベクトル (接ベクトル) $\mathbf{t}$ を追加の頂点属性として渡すことにします。
...
このプログラムオブジェクトの attribute 変数 `tangent` と uniform 変数 `texture` の場所を調べておきます。
...
法線マップのテクスチャを作成し、高さマップを読み込んで、そこに法線マップを作成します。
...
// ラスタライザから受け取る視点座標の補間値
varying vec3 position;

// ラスタライザから受け取る頂点の視点座標系の法線ベクトルの補間値
varying vec3 normal;
...
[^1] これはオリジナルの GLUT の話です。[FreeGLUT](https://freeglut.sourceforge.net/) では直っていた気がします。
