...
![大学から南側]({{ site.baseurl }}/assets/images/20140805_0.jpg)

- [サンプルプログラム (3)](https://github.com/tokoik/image2)

## 背景差分

画像間の演算の例として、もっとも簡単だと思われる背景差分を考えてみます。まず、二つのテクスチャを準備し、その内容を黒 (R, G, B がいずれも 0) で初期化します。テクスチャをクリアする [`glClearTexImage()`](https://registry.khronos.org/OpenGL-Refpages/gl4/html/glClearTexImage.xhtml) や [`glClearTexSubImage()`](https://registry.khronos.org/OpenGL-Refpages/gl4/html/glClearTexSubImage.xhtml) は OpenGL 4.4 の機能のため、OS X では使えません。代わりに FBO を使うなどいくつかの方法があるみたいなんですけど、ここでは考えるのがめんどくさいんで、そういうデータを作ってしまいます。
...
あとは[前回]({{ site.baseurl }}{% post_url 2014-08-01-glsl %})同様、この二つのテクスチャをマッピングして矩形を描画します。
...
![空飛ぶみかんぼうや]({{ site.baseurl }}/assets/images/20140805_1.jpg)

スペースキーをタイプすると、その瞬間にキャプチャされた画像が `image1` に入ります。これがその後にキャプチャされる画像から引かれます。

![みかんぼうやの背景]({{ site.baseurl }}/assets/images/20140805_2.jpg)

いい加減なことをしているので、背景は真っ黒になってません。

![乱入するみかんぼうや]({{ site.baseurl }}/assets/images/20140805_3.jpg)
