...
前の例では、反射方向にあるテクスチャのサンプル値と屈折方向にあるテクスチャのサンプル値の配分を 0.5 に固定していましたが、この配分は、実際には視線の入射角と屈折率の比 $\eta=n_1 / n_2$ で決まります。この関係は[フレネルの式]({{ site.baseurl }}{% post_url 2005-06-20-texture %})で求めることができます。以前、シェーダを使わずにこれを実装するために、この式の値を[１次元テクスチャに格納しておく手法]({{ site.baseurl }}{% post_url 2005-06-20-texture %})を用いましたが、シェーダが使えればこれをシェーダで計算することができます。ただし、フレネルの式は少し複雑なので、シェーダでは Schlick による近似（この人は [Phong の陰影付けモデル]({{ site.baseurl }}{% post_url 2005-10-20-glsl %})を始め、いろんな式の近似式を提案している人ですね）が用いられるようです。

$$F_0=\frac{\displaystyle\left(1-\frac{n_1}{n_2}\right)^2}{\displaystyle\left(1+\frac{n_1}{n_2}\right)^2}$$
$$F=F_0+\left(1-F_0\right)\left(1-\mathbf{N}\cdot\mathbf{V}\right)^5$$

ここで $F_0$ は視線が境界面に垂直に入射するときの反射率で、これは[フレネルの式]({{ site.baseurl }}{% post_url 2005-06-20-texture %})において入射角 $\theta = 0$ ($c = \mathbf{v}\cdot\mathbf{h} = 1$) として求めることができます。$\mathbf{V}$ は視線ベクトルですが、ここでは参照点から視点に向かう単位ベクトルです。$\mathbf{N}$ は参照点における単位法線ベクトルです。$\eta$=`eta` として、これらをもとに参照点における反射率を求め、varying 変数 `t` に格納します。これをバーテックスシェーダプログラムに組み込むと、次のようになります。
