...
の $k$ が負になることがあります。その場合 `refract()` は $\mathbf{0}$ ベクトルを返すので、ガラスの中に不透明な部分が発生して、ちょっと見苦しくなってしまいます。そのため、**大嘘ですが**、`eta` を $\eta$ の逆数にしてごまかしています。フラグメントシェーダプログラム reflect.frag では、この `refract()` によって求めた `s` を使ってキューブマップをサンプリングします。
