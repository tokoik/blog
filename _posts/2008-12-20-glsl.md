...
視線の屈折ベクトルは、視線ベクトル v と法線ベクトル n を `refract()` の引数に与えて得ることができますが、`refract()` は `reflect()` と異なり、v を正規化しておく必要があります。`eta` は視線が通過する境界の前後にある媒質の屈折率の比で、視線が大気中からガラスに進入する場合、大気の屈折率 $n_1 \approx 1.0$、ガラスの屈折率 $n_2 \approx 1.5$ として、$n_1 / n_2 \approx 0.67$ を設定しています。これらから得られた視線の反射ベクトルにテクスチャマトリクスを乗じて、結果を varying 変数 s に格納します。
...
前の例では、反射方向にあるテクスチャのサンプル値と屈折方向にあるテクスチャのサンプル値の配分を 0.5 に固定していましたが、この配分は、実際には視線の入射角と屈折率の比
...
ここで $F_0$ は視線が境界面に垂直に入射するときの反射率で、これは[フレネルの式]({{ site.baseurl }}{% post_url 2005-06-20-texture %})において入射角 $\theta = 0$ ($c = \mathbf{v}\cdot\mathbf{h} = 1$) として求めることができます。$\mathbf{V}$ は視線ベクトルですが、下記では `v` は視点から参照点に向かう単位ベクトルなので、反転して用いています。$\mathbf{N}$ は参照点における単位法線ベクトルです。$\eta$=`eta` として、これらをもとに参照点における反射率を求め、varying 変数 `t` に格納します。これをバーテックスシェーダプログラムに組み込むと、次のようになります。

> 空気中からガラスに光が侵入する場合、屈折率の比 $\eta$ は約 1.52 なるのが本当なのですが、$\eta$ が 1 を超えると `refract()` が 0 ベクトルを返すことがあって（内部反射が全反射する場合）、ちょっと見苦しくなるので逆数にしてごまかしています。
