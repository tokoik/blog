...
メインプログラムの関数 `init()` の始めの方にあるデータ型 `Position` および `Edge` の定義と変数 `position` および `edge` の宣言 (下記の...の部分) を削除します。
...
- [【解答例】]({{ site.baseurl }}/assets/texts/wiresphere.txt)←うーん、きれいに書けなかった

これが出たら、メインプログラムに `wireSphere()` の宣言を追加し、`wireCube()` を呼び出している部分を `wireSphere()` に置き換えてください。`slices` と `stacks` には、ともに 3 以上の整数を設定してください。ここでは 16 と 8 を設定しています。
...
- [summer06 ソースファイル](https://github.com/tokoik/summer06)
