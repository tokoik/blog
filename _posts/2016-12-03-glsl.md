...
```
 ...
 これを使って前後のテクスチャの混合比を求めます。これには組み込み関数 [smoothstep()](https://www.opengl.org/sdk/docs/man/html/smoothstep.xhtml) を使うと便利です。この edge0 を -0.02 程度、edge1 を 0.02 程度にしておけば、angle が負のところでは 0、正のところでは 1 になり、それらが 0 付近で[三次 Hermite 曲線]({{ site.baseurl }}{% post_url 2015-01-05-glsl %})によりなだらかに結ばれます。
