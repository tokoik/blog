...
求めた変換行列 `toView` を用いて法線マップをサンプリングして得た法線ベクトルを変換し、視点座標系の法線ベクトル `fnormal` を求めます。。
...
この法線ベクトル `normal` と視点座標系の光線ベクトル `light` から、拡散反射率 `diffuse` を求めます。また、中間ベクトル `halfway` も視点座標系の光線ベクトル `light` と視線ベクトル `view` を正規化したものから求めます。
