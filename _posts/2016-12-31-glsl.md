...
- [頂点単位の陰影付けを行うプログラム](https://github.com/tokoik/irmap/tree/pervertex)
 ...
 この手法は処理をスクリーン空間で行いますので、このプログラムを<%=a "画素単位の陰影付け|20051007" %>に変更します。上のプログラムのバーテックスシェーダのソースプログラム ([pass1.vert](https://github.com/tokoik/irmap/blob/pervertex/irmap/pass1.vert)) は次にようになっています。
...
![cosine lobe の立体形状]({{ site.baseurl }}/assets/images/20161231_10.png)
 ...
 これを使って $\left[0, 1\right]$ の範囲の一様乱数 $u$ から $\theta_i\larr F^{-1}\left(u\right)$ を求め、それをもとにベクトル $\mathbf{l}\left(\theta_i\right)=\left(x_i, y_i, z_i\right)$ を得ます。ここで $u$ が $\left[0, 1\right]$ の範囲の一様乱数なら $1 - u$ も同じ分布の一様乱数になります。また、$v$ も $\left[0, 1\right]$ の範囲の一様乱数です。
...
環境のテクスチャをサンプリングする点群 (光線方向ベクトル $\mathbf{l}$ を、オブジェクト表面の法線方向に向ける回転の変換行列を求めます。まず、視点座標系の z 軸 $z = \left(0, 0, 1\rhgt)$ と法線ベクトル $mathbf{n}$ に直交するベクトル $\mathbf{z}_n$ を求め、これを使って接空間の接線ベクトル $\mathbf{t} および従接線ベクトル $\mathbf{b}$ を求めます。そして、この $\left(\mathbf{t}, \mathbf{b}, \mathbf{n}\right)$ により回転の変換行列 $\mathbf{m}$ を求めます。ただし、この方法だと法線ベクトルが視点座標系の z 軸に近いとき (回転がわずかなとき) に回転の変換行列の誤差が大きくなるようで、陰影に「へそ」ができてしまいます。何か間違っているのかもしれませんが、回転行列の算出方法を変更したほうがいいような気がします。
...
> 背景を合成してみた <a href="https://t.co/2kusrRB2Gi">pic.twitter.com/2kusrRB2Gi</a></p>&mdash; 床井浩平 (@tokoik) <a href="https://twitter.com/tokoik/status/816933655469694976">2017年1月5日</a>
