...
- [頂点単位の陰影付けを行うプログラム](https://github.com/tokoik/irmap/tree/pervertex)
 ...
 この手法は処理をスクリーン空間で行いますので、このプログラムを[画素単位の陰影付け]({% post_url 2005-10-07-glsl %})に変更します。上のプログラムのバーテックスシェーダのソースプログラム ([pass1.vert](https://github.com/tokoik/irmap/blob/pervertex/irmap/pass1.vert)) は次にようになっています。
...
まず、二回目のパスで使用するポリゴンを準備します。これに最初のパスでフレームバッファオブジェクトにレンダリングした内容をテクスチャとしてマッピングします。このポリゴンの描画に用いる**頂点配列オブジェクト (Vertex Array Object, VAO)** には、[矩形の書き方]({% post_url 2016-08-31-glsl %})で説明した手法を使って**頂点バッファオブジェクト (Vertex Buffer Object, VBO)** を組み込まず、バーテックスシェーダで頂点位置を生成することにします。
...
```
 ...
 環境のテクスチャのサンプリング方法は、「[魚眼レンズ画像の平面展開]({% post_url 2016-06-29-glsl %})」および「[魚眼レンズ画像の平面展開のサンプルプログラム]({% post_url 2016-12-03-glsl %})」の手法に準じています。
