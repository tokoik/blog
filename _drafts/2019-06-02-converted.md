...
<blockquote>window.submit(false) とするとミラー表示を行わなくなるので，その後に glBindFramebuffer(`GL_FRAMEBUFFER`, 0) で通常のフレームバッファへの書き込みに戻してから描画を行うこともできます．ビューポートを元に戻すには window.resetViewport() が使えます．この場合は描画の最後に window.swapBuffers() を呼んでください．</blockquote>
