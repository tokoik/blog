...
ずいぶん前 (今見たら 8 年前!) に [3D テクスチャを使ったプログラム]({% post_url 2006-06-23-post %})を書いた<%= fn "この記事のコメントに OpenGL / GLSL には逆行列を求める方法が無いって書いてますけど，今は GLSL に inverse() っていう組み込み関数がありますね．" %>んですけど，その時に「これでボリュームレンダリングもできるじゃん」って思いました．でも，その方法は (当時使っていた) 教科書にも載ってる良く知られた方法だったので，結局実装しないまま時間が経ってしまいました．先日来 Oculus Rift で[遊んでいて]({% post_url 2014-09-07-post %})，Oculus で雲の中に突入したらどんな感じかなと思って作ってみました．
...
ボリュームデータは CPU 側で作りました (main.cpp の makeVolume() 関数)．ボリュームのサイズが 32 × 32 × 32 だとデータの作成にあまり時間はかかりませんが，128 × 128 × 128 だと結構待たされます．GLSL にも [noise 関数](https://www.opengl.org/sdk/docs/man/html/noise.xhtml)はあるので，それを使ってシェーダで作るべきなんでしょうけど，ここでは前述の[以前作ったプログラム]({% post_url 2006-06-23-post %})を流用しました．条件コンパイルってのは今風ではないんですけど，こらえてつかあさい．
