...
描画する図形は[以前]({% post_url 2009-12-25-post %})と同様に点 (GL_POINTS) にします．頂点バッファオブジェクトを作成し，それに点の位置の初期値を設定します．点の位置の初期値は乱数により決定します．この初期値は CPU 側からは変更することがないので，glBufferData() の第 4 引数 usage は GL_STATIC_DRAW でいいんじゃないかと思います．
