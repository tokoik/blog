...
glDrawElements() を使って図形を描画します. これには２つのバッファオブジェクトが必要になります. 変数 buffers を２つの要素の配列に変更します.
...
<pre>  /* 頂点バッファオブジェクトに８頂点分のメモリ領域を確保する */
...
<pre>  /* 頂点バッファオブジェクトに１２稜線分のメモリ領域を確保する */  
...
<pre>  /* 頂点バッファオブジェクトの指標として buffer[1] を指定する */
glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffer[1]);
```
