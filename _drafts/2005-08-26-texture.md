...
`GL_DOT3_RGB` あるいは `GL_DOT3_RGBA` による内積計算は，テクスチャメモリに格納されている単位ベクトルに対して，この変換が行われていることを考慮します．したがって，これらによる画素の内積計算は，２つのテクスチャ $a_{0\left\{r,g,b\right\}}$ と $a_{1\left\{r,g,b\right\}}$ に対して，次式により行われます．
