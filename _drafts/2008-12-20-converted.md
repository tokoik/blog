...
## ここで f は視線が境界面に垂直に入射するときの反射率で，これは[フレネルの式](20050620)において入射角θ= 0 (c = $\mathbf{v}$·$\mathbf{h}$ = 1) として求めることができます．**V** は視線ベクトルですが，ここでは参照点から視点に向かう単位ベクトルです．**N** は参照点における単位法線ベクトルです．これらをもとに参照点における反射率を求め，`varying` 変数 t に格納します．これをバーテックスシェーダプログラムに組み込むと，次のようになります．
