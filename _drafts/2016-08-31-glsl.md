...
同様にして、[前回]({{ site.baseurl }}{% post_url 2016-06-29-glsl %})の最後に示したようなメッシュは、次のようにして描くことができます。まず、メッシュの幅 slices と高さ stacks から、メッシュの１段分の GL_TRIANGLE_STRIP の頂点数 vertexCount と、その繰り返し回数（段数）primitiveCount を求めます。
