...
以下の項目のみを記載します（**日付は記事のファイル名から自動取得されるため、時差による permalink の不一致を防ぐために `date:` フィールドは含めないでください**）：
...
- **シリーズ連載記事**: 連載名を `categories` または `category` に設定し、関連タグ（`OpenGL`, `GLSL`, `ゼミ`, `FBO` 等）を `tags` に併記します。
- **単発の資料記事**: tDiary のカテゴリ `[OpenGL]` → `category: 資料`、`tags: [OpenGL]`（必要に応じて `GLSL` 等も追加）
- **雑記・メモ記事**: tDiary のカテゴリ `[雑文]` や `[メモ]` → `category: 雑記`
- **日付**: 記事ファイル名（`YYYY-MM-DD-slug.md`）の日付を使用するため、Front Matter には `date:` を記載しません。
